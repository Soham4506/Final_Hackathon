...
export const NotificationsPage: React.FC = () => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white border border-[#76777d]/20 rounded-2xl p-5 sm:p-6 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-[#76777d]">Citizen In-App Notifications</span>
          </div>
      ...
      {/* Notifications List */}
      <div className="bg-white border border-[#76777d]/20 rounded-2xl overflow-hidden shadow-xs divide-y divide-[#76777d]/15 text-xs">
        {notifications.length === 0 ? (
          <div className="p-12 text-center text-[#76777d]">No operational alerts in feed.</div>
        ) : (
          notifications.map((n) => (
            <div
              key={n.id}
              ...
              className={`p-5 transition-colors flex items-start gap-4 cursor-pointer hover:bg-slate-50 ${
                n.isRead ? 'opacity-60 bg-[#fcf8fa]' : 'bg-white'
              }`}
            >
              ...
              <div className="flex-1 space-y-1.5">
                <div className="flex items-center justify-between gap-2">
                  <span className="font-bold text-[#1b1b1d] text-sm">{n.title}</span>
                ...
                <p className="text-[#57657b] leading-relaxed text-xs">{n.message}</p>
