    ...
    <div className="space-y-5">
      ...
      <div className="bg-card border border-border rounded-2xl p-5 sm:p-6 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-muted-foreground">KMC Council Overview</span>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-card p-4 rounded-xl border border-border shadow-xs flex items-center justify-between">
          <div>
            <span className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Estimated Demand</span>
            <div className="text-2xl font-bold text-foreground font-mono mt-1">
              ₹{(totalCostEstimated / 1000).toFixed(1)}k
            </div>
            <p className="text-[10px] text-muted-foreground">Active issue demand</p>
          </div>
        ...
        <div className="bg-card p-4 rounded-xl border border-border shadow-xs flex items-center justify-between">
          <div>
            <span className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">AI Intake Accuracy</span>
            <div className="text-2xl font-bold text-emerald-700 font-mono mt-1">
              {avgConfidence}%
            </div>
            <p className="text-[10px] text-muted-foreground">Verification score</p>
          </div>
        ...
        <div className="bg-card p-4 rounded-xl border border-border shadow-xs flex items-center justify-between">
          <div>
            <span className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Total Wards Monitored</span>
            <div className="text-2xl font-bold text-blue-700 font-mono mt-1">
              {zones.length} Wards
            </div>
            <p className="text-[10px] text-muted-foreground">Kopargaon Municipal Grid</p>
          </div>
        ...
        <div className="bg-card p-4 rounded-xl border border-border shadow-xs flex items-center justify-between">
          <div>
            <span className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Critical Threats (P0)</span>
            <div className="text-2xl font-bold text-[#ba1a1a] font-mono mt-1">
              {issues.filter((i) => (i.priorityScore?.finalScore ?? 0) >= 80).length}
            </div>
            <p className="text-[10px] text-muted-foreground">Immediate escalation</p>
          </div>
